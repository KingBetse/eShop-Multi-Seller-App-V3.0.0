        ...
        child: Column(
          children: [
            CardWithTitleDivider(
                ...
                child: Column(
                  children: [
                    buildDetailRow('ORDER_ID_LBL'.translate(context: context),
                        "#${widget.order.id!}"),
                    buildDetailRow('Order Date'.translate(context: context),
                        widget.order.dateAdded!),
                    buildDetailRow('Payment method'.translate(context: context),
                        widget.order.paymentMethod!),
                    if (widget.order.notes != '')
                      buildDetailRow('Notes'.translate(context: context),
                          widget.order.notes!),
            ...
            CardWithTitleDivider(
                title: 'PRODUCTS'.translate(context: context),
                          ...
                          ? Row(
                              children: [
                                ...
                                MaterialButton(
                                  ...
                                  child: Text(
                                      'send_email'.translate(context: context)),
                                ),
                              ],
                            )
                          : null,
            ...
            if (widget.order.orderattachments!.isNotEmpty)
              Column(
                children: [
                  CardWithTitleDivider(
                    title: 'ORDER_ATTACHMENTS'.translate(context: context),
                    child: Column(
            ...
            CardWithTitleDivider(
                title: widget.order.isDigitalOrder
                    ? 'contact_details'.translate(context: context)
                ...
                child: Column(
                  ...
                  children: [
                    ...
                    Text(
                        '${"phone_number".translate(context: context)}: ${widget.order.mobile}'),
                  ],
                )),
            const SizedBox(
              height: 15,
            ...
            CardWithTitleDivider(
                title: 'price_details'.translate(context: context),
                child: Column(
                  crossAxisAlignment: CrossAxisAlignment.start,
                  children: [
                    buildDetailRow(
                        'price'.translate(context: context),
                    ...
                    buildDetailRow(
                        'promocd'.translate(context: context),
                        DesignConfiguration.getPriceFormat(
                            context, widget.order.promoDiscount!.toDouble())!),
                    buildDetailRow(
                        'wallet_balance'.translate(context: context),
                        DesignConfiguration.getPriceFormat(
...
class _DigitalUpdateStatusDialogState extends State<DigitalUpdateStatusDialog> {
  ...
  Widget build(BuildContext context) {
    return AlertDialog(
      title: Text('update_status'.translate(context: context)),
      content: Column(
        ...
        children: [
          RadioListTile(
            ...
            groupValue: status,
            title: Text('received'.translate(context: context)),
            onChanged: (value) {
              setState(() {
                status = value!;
              });
          ...
          RadioListTile(
            ...
            controlAffinity: ListTileControlAffinity.trailing,
            groupValue: status,
            title: Text('delivered'.translate(context: context)),
            onChanged: (value) {
              setState(() {
                status = value!;
              });
      ...
      actions: [
        ...
        TextButton(
            ...
            child: Text('Update'.translate(context: context))),
      ],
    );
  }
}
