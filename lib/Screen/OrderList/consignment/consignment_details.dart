...
class _ConsignmentDetailsState extends State<ConsignmentDetails> {
  ...
  void _onTapUpdateStatus() async {
    if (selectedOrderStatus!.name == 'delivered' &&
      ...
      String? enteredOtp = await showPopUpViewForOTP(context); // Wait for OTP
      if (enteredOtp == null || enteredOtp.isEmpty) {
        return; // Do nothing if OTP is not entered
      }

      otpController.text = enteredOtp; // Store entered OTP
    }
      ...
      body: BlocListener<UpdateConsignmentStatusCubit,
        ...
        listener: (context, state) {
          if (state is UpdateConsignmentStatusInSuccess) {
            ...
            setState(() {});
            setSnackbar(
                'PARCEL_STATUS_UPDATE_SUCCESSFULLY'.translate(context: context),
                context);
          }
            ...
            child: Column(
              children: [
                ...
                CardWithTitleDivider(
                    ...
                    child: Column(
                      children: [
                        buildDetailRow('parcel_id'.translate(context: context),
                            widget.consignment.id),
                        buildDetailRow(
                            'parcel_date'.translate(context: context),
                            widget.consignment.createdDate),
                        buildDetailRow(
                            'PAYMENT_MTHD'.translate(context: context),
                        ...
                        if (widget.consignment.trackingDetails != null &&
                          ...
                          buildDetailRow(
                              'shiprocket_order_id'.translate(context: context),
                ...
                CardWithTitleDivider(
                    ...
                    child: Column(
                      ...
                      children: [
                        ...
                        Text(
                            '${"phone_number".translate(context: context)}: ${widget.consignment.mobile}'),
                      ],
                    )),
                const SizedBox(
                  height: 10,
  ...
  Future<String?> showPopUpViewForOTP(BuildContext parentContext) async {
            ...
            TextButton(
              onPressed: () {
                if (formKey.currentState!.validate()) {
                  Navigator.of(context).pop(otpController.text); // Return OTP
                }
              },
